...
function recipe_part_1(c_max)
  ...
  while c < c_max
    ...
    te,re,ti,ri,kill_flag = euler_lif_NA(h,runtime,Ne,Ni,wee,wei,wie,wii,s_strength,s_strength, vth, 20., 2., 2., 2., 2.);
    ...
    if ((top_e_neurons == -5) & (bot_e_neurons == -5))
      ...
      #FF
      ntfa = network_fano(te_pt, re_pt, cbinsize, Int64(ntotal))
      ...
      c+=1
      if (2. > E_CV_mean_top > .8) & (2. > E_CV_mean_bot > .8) & (2. > I_CV_mean > .8) & (E_spike_correlation_top < .1) & (E_spike_correlation_bot < .1) & (I_spike_correlation < .1)
        success = true
        println("##RESULT success_1, $(c), $(E_rate), $(I_rate), $(wta_ness), $(bias), $(ntfa), $(E_FANO_mean_top), $(E_FANO_mean_bot), $(I_FANO_mean), $(E_CV_mean_top), $(E_CV_mean_bot), $(I_CV_mean), $(E_spike_correlation_top), $(E_spike_correlation_bot), $(I_spike_correlation), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(Ne)")
        return c, kee, kei, kie_L, kii, Aee*p, Aei*p, Aie_L*p, Aii*p, wta_ness
      else
        println("##RESULT $(success), $(c), $(E_rate), $(I_rate), $(wta_ness), $(bias), $(ntfa), $(E_FANO_mean_top), $(E_FANO_mean_bot), $(I_FANO_mean), $(E_CV_mean_top), $(E_CV_mean_bot), $(I_CV_mean), $(E_spike_correlation_top), $(E_spike_correlation_bot), $(I_spike_correlation), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(Ne)")
      end
    end
    if c >= c_max -1
...
function recipe_part_2(kee, kei, kie_L, kiim, Aee, Aei, Aie_L, Aii, c_max)
  ...
  while c < c_max
    ...
    if ((top_e_neurons == -5) & (bot_e_neurons == -5))
      ...
      println("##RESULT $(success), $(c), $(E_rate), $(I_rate), $(wta_ness), $(bias), $(ntfa), $(E_FANO_mean_top), $(E_FANO_mean_bot), $(I_FANO_mean), $(E_CV_mean_top), $(E_CV_mean_bot), $(I_CV_mean), $(E_spike_correlation_top), $(E_spike_correlation_bot), $(I_spike_correlation), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(Ne)")
      if bias == 1
        if (2. > E_CV_mean_top > .8) & (2. > I_CV_mean > .8) & (E_spike_correlation_top < .1) & (wta_ness > .8)
          success = true
          #plot(te, re, "g.")
          println("##RESULT success_2, $(c), $(E_rate), $(I_rate), $(wta_ness), $(bias), $(ntfa), $(E_FANO_mean_top), $(E_FANO_mean_bot), $(I_FANO_mean), $(E_CV_mean_top), $(E_CV_mean_bot), $(I_CV_mean), $(E_spike_correlation_top), $(E_spike_correlation_bot), $(I_spike_correlation), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(Ne)")
          return c, kee, kei, kie_L, kii, Aee*p, Aei*p, Aie_L*p, Aii*p, wta_ness
        end
      else bias == 2
          if (2. > E_CV_mean_bot > .8) & (2. > I_CV_mean > .8) & (E_spike_correlation_bot < .1) & (wta_ness > .8)
            success = true
            plot(te, re, "g.")
            println("##RESULT success_2, $(c), $(E_rate), $(I_rate), $(wta_ness), $(bias), $(ntfa), $(E_FANO_mean_top), $(E_FANO_mean_bot), $(I_FANO_mean), $(E_CV_mean_top), $(E_CV_mean_bot), $(I_CV_mean), $(E_spike_correlation_top), $(E_spike_correlation_bot), $(I_spike_correlation), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(Ne)")
            return c, kee, kei, kie_L, kii, Aee*p, Aei*p, Aie_L*p, Aii*p, wta_ness
...
function recipe_part_2b(kee, kei, kie_L, kii, Aee, Aei, Aie_L, Aii, c_max)
  ...
  Aie_L /= p
  Aii /= p
  kee=sd_2_k(rand(15:35)/100.)
  Aees = linspace(50, 250, c_max)
...
function recipe_part_3(kee, kei, kie_L, kii, Aee, Aei, Aie_L, Aii, c_max)
  ...
  for i in g
    ...
    te,re,ti,ri,kill_flag, e_top, e_bot, aet, aeb, ev = euler_lif(h,runtime,Ne,Ni,wee,wei,wie,wii,s_strength,s_strength,i,tauadapt);
    ...
    if (tdom > 20000) & (bdom > 20000)
      ...
      countfaT = count_train_intron(fbinsize, ttf, rtf, TN, length(TN), false)
      countfaB = count_train_intron(fbinsize, tbf, rbf, BN, length(BN), false)
      ...
      MFT, medFT, stdFT = fano_train(countfaT, -5)
      MFB, medFB, stdFB = fano_train(countfaB, -5)
      mcvt, medcvt, stdcvt = CV_ISI(top, TN, te, re)
      mcvb, medcvb, stdcvb = CV_ISI(bot, BN, te, re)
      if (mcvt > .8) & (mcvt < 2.5) & (mcvb > .8) & (mcvb < 2.5) & (cwT < .1) & (cwB < .1) & (MD > 5000)
        println("party time")
        success = true
    ...
    elseif (tdom > 180000) | (bdom > 180000)
      ...
      countfaB = count_train_intron(fbinsize, tbf, rbf, Neurons, length(Neurons), false)
...
c_wta, kee, kei, kie_L, kii, Aee, Aei, Aie_L, Aii, wta_ness = recipe_part_2b(kee, kei, kie_L, kii, Aee, Aei, Aie_L, Aii, 1000)
end
if kee == -5
  exit()
else
...
''''
graph says ee \propto ei*ie for WTA
learning rule says given ei, ie for irg, walk sideways on the EE until you hit the red region
