...
if (tdom > ntotal*.1) & (bdom > ntotal*.1)
  ...
  println("##RESULT 1, $(MDT), $(MDB), $(MD), $(cwT), $(cwB), $(mtu), $(stdtu), $(sktu), $(kutu), $(mbd), $(stdbd), $(skbd), $(kubd), $(mtd), $(stdtd), $(sktd), $(kutd), $(mbu), $(stdbu), $(skbu), $(kubu), $(MFT), $(medFT), $(stdFT), $(MFB), $(medFB), $(stdFB), $(mcvt), $(medcvt), $(stdcvt), $(mcvb), $(medcvb), $(stdcvb), $(kee), $(kei), $(kie_L), $(kii), $(Aee*p), $(Aei*p), $(Aie_L*p), $(Aii*p), $(s_strength), $(p), $(g_e)")
end
...
figure(1)
plot(te, re, "g.")
title("cwT=$(cwT), cwB=$(cwB), MFT=$(MFT), MFB=$(MFB), cvt=$(mcvt), cvb=$(mcvb)", fontsize = 24)
xlabel("kee=$(kee), kei=$(kei), kie=$(kie_L), kii=$(kii), Aee=$(Aee*p), Aei=$(Aei*p), Aie=$(Aie_L*p), Aii=$(Aii*p), S=$(s_strength), p=$(p), g=$(g_e)", fontsize = 18)
# figure(2)
# plot(e_top, "r")
# x=te[find(re.==rt)]
